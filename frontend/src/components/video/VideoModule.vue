...
<template>
  <div class="video-module h-full flex flex-col bg-base-200">
    <!-- Pre-call lobby -->
    <div
      v-if="callState === 'idle'"
    ...
    >
      <div class="text-center">
        <h2 class="text-2xl font-bold mb-2">Rejoindre la visioconference</h2>
      ...
      <!-- Preview -->
      <div class="relative w-80 aspect-video bg-base-300 rounded-xl overflow-hidden shadow-lg">
        <video
          ref="localVideoRef"
          autoplay
          muted
        ...
        >
          <div class="avatar placeholder">
            <div class="bg-neutral text-neutral-content rounded-full w-20">
              <span class="text-2xl">{{ authStore.user?.displayName?.charAt(0) }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex gap-4">
        <button
          ...
          :class="localMedia.audioEnabled ? 'btn-ghost' : 'btn-error'"
          @click="toggleAudio"
          title="Microphone"
        >
          <svg v-if="localMedia.audioEnabled" class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button
          class="btn btn-circle btn-lg"
          :class="localMedia.videoEnabled ? 'btn-ghost' : 'btn-error'"
          @click="toggleVideo"
          title="Camera"
        >
          <svg v-if="localMedia.videoEnabled" class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <button class="btn btn-circle btn-lg btn-ghost" @click="showSettings = true" title="Parametres">
      ...
      <button class="btn btn-primary btn-lg gap-2" @click="joinCall">
    ...
    <template v-else-if="callState === 'connected'">
      ...
      <div class="flex-1 p-4 overflow-auto">
        ...
        >
          <!-- Local video -->
          <VideoTile
            :stream="localMedia.screenSharing ? localMedia.screenStream : localMedia.videoStream"
      ...
      <div class="bg-base-300 p-4">
        <div class="flex items-center justify-center gap-4">
          ...
          <button
            class="btn btn-circle btn-lg"
            :class="localMedia.screenSharing ? 'btn-success' : 'btn-ghost'"
            @click="localMedia.screenSharing ? stopScreenShare() : startScreenShare()"
    ...
    >
      <div class="text-center">
        ...
        <p class="mt-4">Connexion en cours...</p>
      </div>
    </div>
    ...
    <dialog class="modal" :class="{ 'modal-open': showSettings }">
      <div class="modal-box">
        <h3 class="font-bold text-lg mb-4">Parametres audio/video</h3>

        <div class="form-control mb-4">
          <label class="label">
            <span class="label-text">Microphone</span>
          </label>
          <select v-model="selectedAudioInput" class="select select-bordered w-full">
        ...
        <div class="form-control mb-4">
          ...
          <select v-model="selectedVideoInput" class="select select-bordered w-full">
        ...
        <div class="modal-action">
          <button class="btn" @click="showSettings = false">Fermer</button>
        </div>
      </div>
      <form method="dialog" class="modal-backdrop" @click="showSettings = false">
        <button>close</button>
      </form>
    </dialog>
  </div>
