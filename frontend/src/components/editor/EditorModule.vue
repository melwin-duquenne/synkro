...
function initEditor() {
  ...
  editor.value = new Editor({
    ...
    editorProps: {
      attributes: {
        class: 'prose prose-sm sm:prose max-w-none focus:outline-none min-h-[400px] p-6'
      }
    },
    onUpdate: () => {
...
<template>
  <div class="editor-module h-full flex flex-col bg-base-100">
    <!-- Toolbar -->
    <div v-if="editor" class="toolbar flex flex-wrap items-center gap-1 p-2 border-b border-base-300 bg-base-200">
      <!-- Text formatting -->
      ...
      <div class="flex items-center gap-2">
        <!-- Save status -->
        <span v-if="saving" class="text-xs text-base-content/60 flex items-center gap-1">
          <span class="loading loading-spinner loading-xs"></span>
          Sauvegarde...
        </span>
        <span v-else-if="lastSaved" class="text-xs text-base-content/60">
