  ...
  <div class="card bg-base-100 shadow-xl">
    <div class="card-body">
      ...
      <div v-if="tasks.today.length > 0" class="mb-4">
        <h3 class="text-sm font-semibold mb-2 text-base-content/70 flex items-center gap-2">
        ...
        <div class="space-y-2">
          <div 
            ...
            :key="task.id"
            class="flex items-start gap-2 p-2 rounded-lg hover:bg-base-200 transition-colors cursor-pointer"
          >
            <input type="checkbox" class="checkbox checkbox-sm mt-1" />
            <div class="flex-1 min-w-0">
              <div class="font-medium truncate">{{ task.title }}</div>
              <div v-if="task.room" class="text-xs text-primary flex items-center gap-1">
      ...
      <div v-if="tasks.overdue.length > 0" class="mb-4">
        <h3 class="text-sm font-semibold mb-2 text-error flex items-center gap-2">
          ...
          <span>En retard</span>
          <span class="badge badge-error badge-sm">{{ tasks.overdue.length }}</span>
        </h3>
        <div class="space-y-2">
          <div 
          ...
          >
            <input type="checkbox" class="checkbox checkbox-sm checkbox-error mt-1" />
            <div class="flex-1 min-w-0">
              <div class="font-medium truncate">{{ task.title }}</div>
      ...
      <div v-if="tasks.roomProgress.length > 0">
        <h3 class="text-sm font-semibold mb-3 text-base-content/70">Progression des projets</h3>
        <div class="space-y-3">
          <div v-for="room in tasks.roomProgress" :key="room.roomId">
            <div class="flex justify-between items-center mb-1">
              <span class="text-sm font-medium truncate">{{ room.roomName }}</span>
      ...
      <div v-if="tasks.today.length === 0 && tasks.overdue.length === 0 && tasks.roomProgress.length === 0" 
