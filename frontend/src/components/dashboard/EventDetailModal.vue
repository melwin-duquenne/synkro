<template>
  <Teleport to="body">
    <Transition name="modal">
      ...
      >
        <div
          class="w-full max-w-2xl bg-[#0a1628] rounded-2xl shadow-2xl overflow-hidden"
          style="max-height: 90vh; display: flex; flex-direction: column"
          @click.stop
        >
          <!-- Header -->
          <div
            class="bg-gradient-to-r from-[#4115df] to-[#6a3fe8] p-6 text-white relative"
          >
            <button
          ...
          >
            ...
            <div class="flex items-start gap-4">
              <div
                v-html="getEventIcon(event.eventType)"
                class="flex-shrink-0 mt-1"
              ></div>
              <div class="flex-1">
                <h4 class="text-2xl font-bold mb-2" style="color: inherit">
                  {{ event.title }}
                </h4>
                <span
            ...
            <!-- Horaires -->
            <div class="bg-[#1a3a52] rounded-xl p-5">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <div class="text-xs uppercase tracking-wide opacity-70 mb-1">
                    Début
                  </div>
              ...
              <div class="mt-3 pt-3 border-t border-[#4115df]/30">
            ...
            <div
              v-if="event.room"
              class="bg-primary/10 rounded-xl p-5 border-2 border-primary/20"
            >
              <h5
                class="font-semibold mb-3 flex items-center gap-2 text-primary"
              >
                <svg
                  class="w-5 h-5"
                  fill="none"
            ...
            <!-- Description -->
            <div v-if="event.description" class="bg-[#1a3a52] rounded-xl p-5">
            ...
            <div v-else class="text-center py-4 text-[#b0b0b0] text-sm italic">
              Aucune description disponible pour cet événement
            </div>
          </div>

          <!-- Footer -->
          <div class="border-t border-[#4115df]/30 p-6 bg-[#050d1a]">
            <div class="flex gap-3 justify-end">
              <button
                class="px-6 py-2 bg-[#1a3a52] hover:bg-[#2a4a62] text-white rounded-lg font-medium transition-colors"
                @click="close"
                type="button"
              >
                Fermer
              ...
              <button
                v-if="event.room"
                class="px-6 py-2 bg-[#4115df] hover:bg-[#6a3fe8] text-white rounded-lg font-medium transition-colors"
                @click="navigateToRoom(event.room.id)"
                type="button"
              >
                Rejoindre la room
...
const getEventIcon = (type: string): string => {
  const icons: Record<string, string> = {
    ...
    absence:
      '<svg class="w-6 h-6 text-error" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" /></svg>',
    reminder:
...
const formatDuration = (start: string, end: string): string => {
  ...
  const [, yearS = "0", monthS = "1", dayS = "1", hourS = "0", minuteS = "0"] =
    matchStart ?? [];
  const [, yearE = "0", monthE = "1", dayE = "1", hourE = "0", minuteE = "0"] =
    matchEnd ?? [];
