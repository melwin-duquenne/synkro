<template>
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="modal-box max-w-2xl">
      <h3 class="font-bold text-lg mb-4">Sélectionner une room pour créer une tâche</h3>
      ...
      <div v-else class="space-y-3 max-h-96 overflow-y-auto">
        <div
          ...
          :key="room.id"
          class="card bg-base-100 border cursor-pointer hover:bg-base-200 transition-colors"
            ...
            <div class="flex items-center justify-between">
              <div>
                ...
                <p class="text-sm text-base-content/60">
                  {{ room.moduleRooms?.length || 0 }} module(s) • {{ room.visibility === 'private' ? 'Privée' : 'Entreprise' }}
                </p>
              </div>
