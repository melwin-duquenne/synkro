<template>
  <div class="dashboard-card">
    <div>
      ...
      <!-- État vide -->
      <div
        v-if="!workload || !workload.daily || workload.daily.length === 0"
      ...
      <template v-else>
        ...
        <div v-if="workload.hasAlert" class="alert alert-warning mb-4">
          ...
          <span
            >{{ workload.overloadDays }} jour(s) de surcharge cette
            semaine</span
          >
        </div>
          ...
          <div>
            ...
            <div
              ...
              :class="{
                ...
                'text-warning':
                  todayWorkload.percentage >= 80 &&
                  todayWorkload.percentage <= 100,
            ...
            <div class="stat-desc">
              / {{ todayWorkload.hoursAvailable }}h ({{
                Math.round(todayWorkload.percentage)
              }}%)
            </div>
            <button
              v-if="todayWorkload.tasks && todayWorkload.tasks.length > 0"
              class="btn btn-xs btn-ghost mt-2 bg-white text-black border-0"
          ...
          <div>
            ...
            <div
              ...
              :class="{
                ...
                'text-warning':
                  weekWorkload.percentage >= 80 &&
                  weekWorkload.percentage <= 100,
            ...
            <div class="stat-desc">
              / {{ weekWorkload.hoursAvailable }}h ({{
                Math.round(weekWorkload.percentage)
              }}%)
            </div>
          </div>
        </div>
        ...
        <!-- Graphique par jour -->
        <div class="space-y-2">
          <h3 class="font-semibold text-sm mb-3 text-white">Charge par jour</h3>
          <div
            v-for="day in workload.daily"
            :key="day.date"
            class="flex items-center gap-3 cursor-pointer hover:bg-[#1a3a52] p-2 rounded-lg transition-colors"
          ...
          >
            ...
            <div class="flex-1">
              <div class="relative h-4 rounded-full overflow-hidden">
                <div
                  class="absolute h-full transition-all duration-300 flex items-center justify-end px-2 text-xs font-bold rounded-full"
                  :class="{
                ...
                >
                  <span v-if="day.percentage > 10" class="text-white"
                    >{{ Math.round(day.percentage) }}%</span
                  >
                </div>
              </div>
            </div>
            <div
              class="w-24 text-right text-xs flex items-center justify-end gap-1"
            >
              <span :class="{ 'text-error font-bold': day.isOverloaded }">
                {{
                  Math.round(
                    (day.hoursUsed ??
                      (day.percentage * (day.hoursAvailable || 9)) / 100) * 10,
                  ) / 10
                }}h / {{ day.hoursAvailable || 9 }}h
              </span>
              <svg
        ...
        <!-- Légende -->
        <div class="flex gap-4 mt-4 text-xs border-base-300 pt-3">
  ...
  <Teleport to="body">
    ...
    >
      <div
        class="bg-[#0a1628] rounded-2xl shadow-2xl max-w-2xl w-full max-h-[80vh] overflow-hidden flex flex-col"
      >
        <!-- Header -->
        <div
          class="bg-gradient-to-r from-[#1e90ff] to-[#1a3a52] p-6 text-white"
        >
        ...
        <div class="flex-1 overflow-y-auto p-6">
          <div class="mb-4">
            <div class="stat bg-[#1a3a52] rounded-lg">
          ...
          >
            <div
              v-for="task in todayWorkload.tasks"
              :key="task.id"
              class="flex items-center justify-between p-3 bg-[#1a3a52] rounded-lg"
            >
              <div class="flex-1">
                <div class="font-medium">{{ task.title }}</div>
                <div
                  class="text-xs text-base-content/70 flex items-center gap-1 mt-1"
                >
                  <svg
                    class="w-3 h-3"
                    fill="none"
    ...
    >
      ...
      >
        <!-- Header -->
        <div
          class="bg-gradient-to-r from-primary to-primary-focus p-6 text-white"
        >
          <div class="flex justify-between items-center">
            <h3 class="text-2xl font-bold">
              Détails - {{ selectedDay.dayName }}
            </h3>
            <button
              class="w-8 h-8 flex items-center justify-center rounded-full hover:bg-white/20 transition-colors"
        ...
        <div class="flex-1 overflow-y-auto p-6">
            ...
            <div class="stat bg-[#1a3a52] rounded-lg">
              ...
              <div
                ...
                :class="{
                  ...
                  'text-warning':
                    selectedDay.percentage >= 80 &&
                    selectedDay.percentage <= 100,
              ...
              <div class="stat-desc">
                {{
                  selectedDay.hoursUsed ??
                  Math.round(
                    ((selectedDay.percentage *
                      (selectedDay.hoursAvailable || 9)) /
                      100) *
                      10,
                  ) / 10
                }}h / {{ selectedDay.hoursAvailable || 9 }}h
              </div>
            </div>
          </div>
          ...
          <h4 class="font-semibold mb-3">Tâches planifiées</h4>
          <div
            v-if="selectedDay.tasks && selectedDay.tasks.length > 0"
            class="space-y-2"
          >
            <div
