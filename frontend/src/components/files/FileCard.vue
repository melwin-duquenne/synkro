...
<template>
  <div
    class="file-card cursor-pointer transition-all duration-200 group"
    :class="{ 'drag-over-folder': isDragOverFolder }"
    draggable="true"
    ...
    <div class="p-4">
      <div class="flex items-start justify-between">
        <div class="flex items-center gap-3 min-w-0 flex-1">
          ...
          <div class="min-w-0">
            <p
              class="font-medium text-sm text-[#e7f0fa] block max-w-full truncate"
              :title="file.fileName"
            >
              {{ truncateName(file.fileName) }}
            </p>
            <div class="flex items-center gap-2 text-xs text-[#8ea4be] mt-1">
              <span v-if="!file.isFolder">{{ formatSize(file.size) }}</span>
              <span v-if="file.isFolder"
                >{{ file.childCount }} item{{
                  file.childCount !== 1 ? "s" : ""
                }}</span
              >
              <span>{{ formatDate(file.createdAt) }}</span>
            </div>
            <p v-if="file.user" class="text-xs text-[#7088a3] mt-0.5">
        ...
        <!-- Dropdown menu -->
        <div
          class="dropdown dropdown-end opacity-0 group-hover:opacity-100 transition-opacity"
        >
          <label tabindex="0" class="menu-btn" @click.stop>
            <svg
              xmlns="http://www.w3.org/2000/svg"
              class="h-4 w-4"
              fill="none"
          ...
          <ul
            tabindex="0"
            class="dropdown-content z-1 menu p-2 shadow-lg menu-panel w-44"
          >
            <li><a @click.stop="emit('rename', file)">Renommer</a></li>
            <li><a @click.stop="emit('move', file)">Deplacer</a></li>
            <li v-if="!file.isFolder">
              <a @click.stop="emit('download', file)">Telecharger</a>
            </li>
            <li>
              <a @click.stop="emit('delete', file)" class="text-error"
                >Supprimer</a
              >
            </li>
          </ul>
...
.file-card:hover {
  ...
  box-shadow: 0 12px 24px rgba(3, 10, 20, 0.35);
}

.drag-over-folder {
  border-color: rgba(110, 210, 158, 0.8);
  background: linear-gradient(
    165deg,
    rgba(22, 53, 47, 0.7),
    rgba(12, 34, 31, 0.82)
  );
  box-shadow: 0 0 0 3px rgba(110, 210, 158, 0.2);
}
