...
const breadcrumbs = ref<{ id: number | null; name: string }[]>([{ id: null, name: 'Racine' }])
...
async function loadFolders(parentId: number | null) {
  ...
  try {
    ...
    const response = await fetch(`${API_URL}/rooms/${props.roomId}/files?${params}`, {
      headers: authStore.getAuthHeaders()
    })

    if (response.ok) {
      const data = await response.json()
      const all: FileResource[] = Array.isArray(data) ? data : (data['hydra:member'] || data.member || [])
...
function handleMove() {
  ...
  const target = selectedFolderId.value !== undefined ? selectedFolderId.value : currentParentId.value
  emit('move', target)
}
...
<template>
  <dialog class="modal" :class="{ 'modal-open': show }">
    <div class="modal-box">
      <h3 class="font-bold text-lg mb-4">Deplacer "{{ file?.fileName }}"</h3>

      <!-- Breadcrumbs -->
      <div class="text-sm breadcrumbs mb-3">
        <ul>
          <li v-for="(bc, i) in breadcrumbs" :key="i">
            <a @click="navigateBreadcrumb(i)" class="cursor-pointer">{{ bc.name }}</a>
          </li>
        </ul>
      </div>

      <!-- Folder list -->
      <div class="border rounded-lg p-2 max-h-60 overflow-y-auto min-h-[120px]">
        ...
        <div
          ...
          :class="{ 'bg-primary/10 border border-primary': selectedFolderId === currentParentId }"
        ...
        >
          ...
          <span class="text-sm italic text-base-content/60">Ici (dossier actuel)</span>
        </div>
        ...
        <template v-if="!loading">
          <div
            ...
            :key="folder.id"
            class="flex items-center gap-2 p-2 rounded cursor-pointer hover:bg-base-200 transition-colors"
            :class="{ 'bg-primary/10 border border-primary': selectedFolderId === folder.id }"
          ...
          >
            ...
            <span class="text-sm">{{ folder.fileName }}</span>
            <span class="text-xs text-base-content/40 ml-auto">{{ folder.childCount }} items</span>
          </div>
          <p v-if="folders.length === 0 && !loading" class="text-sm text-base-content/50 text-center py-4">
            Aucun sous-dossier
          </p>
        </template>
      </div>
